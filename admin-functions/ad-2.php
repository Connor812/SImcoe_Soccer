...
$ad_image = $_FILES["ad_image_2"];
$ad_text = $_POST["ad_text_2"];
$ad_link = $_POST["ad_link_2"];
...
if (!getimagesize($_FILES["ad_image_2"]["tmp_name"])) {
...
if (file_exists($targetFile)) {
    ...
    require_once("../db/db_config.php");
    updateAd2($ad_text, $ad_link, basename($_FILES["ad_image_2"]["name"]), $conn);
    exit;
}

// Move the file to the target directory
if (!move_uploaded_file($_FILES["ad_image_2"]["tmp_name"], $targetFile)) {
    // echo "Error moving file.";
    header("Location: " . BASE_URL . "admin.php?error=moving_file#ad-section");
    exit;
}

function updateAd2($ad_text, $ad_link, $ad_image, $conn)
{
    // Assuming there's only one row in the table
    $sql = "UPDATE `index_page` SET `ad_text_2` = ?, `ad_link_2` = ?, `ad_image_2` = ?;";
